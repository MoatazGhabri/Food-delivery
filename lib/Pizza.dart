...
import 'package:fseg/Burger.dart';
...
class Pizza extends StatelessWidget {
  ...
  final String? number;
  const Pizza({Key? key,this.email, this.name, this.address, this.number}) : super(key: key);
  @override
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        body: ListView(children: [
          AndroidSmall2(email: email, name: name , address: address, number: number),
        ]),
      ),
    );
  }
...
class _AndroidSmall2State extends State<AndroidSmall2> {
  ...
  Future<void> getFoods() async {
    ...
    final foodsReference = databaseReference.child('foods').orderByChild('category').equalTo('Pizza');
